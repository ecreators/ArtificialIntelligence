...
public final class Layer {
   ...
   */
  Layer bindFullMesh(Layer parentLayer) {
  ...
  public Float[] getOutputs() {
    Float[] outputs = neurons.stream().map(n -> {
      System.out.println("neuron.name: " + n.getName());
      Float value = n.getOutputValue();
      if (roundOutputs) {
    ...
    System.out.println(MessageFormat.format("output fetched ({0}):{2}>>{2}{1}{2}<<",
      outputs.length,
      join(asList(outputs), System.lineSeparator()),
      System.lineSeparator()));

    return outputs;
  }

  private static <T> String join(Collection<T> list, String delimiter) {
